...
function pageHref(searchParams: Record<string, string | string[] | undefined>, page: number, basePath: string, hash?: string) {
...
export function Pagination({
  ...
  return (
    <nav aria-label="Paginación" className="mt-8 flex flex-wrap items-center justify-center gap-2">
      <Link
        href={pageHref(searchParams, Math.max(1, page - 1), basePath, hash)}
      ...
      {pages.map((item, index) => {
        ...
        return (
          <span key={item} className="flex items-center gap-2">
            ...
            <Link
              href={pageHref(searchParams, item, basePath, hash)}
      ...
      <Link
        href={pageHref(searchParams, Math.min(pageCount, page + 1), basePath, hash)}
